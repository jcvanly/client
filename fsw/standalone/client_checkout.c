...
** Include Files
*/
#include "client_checkout.h"

/*
** Global Variables
*/
uart_info_t ClientUart;
CLIENT_Device_HK_tlm_t ClientHK;
CLIENT_Device_Data_tlm_t ClientData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = CLIENT_CommandDevice(&ClientUart, CLIENT_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = CLIENT_RequestHK(&ClientUart, &ClientHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("CLIENT_RequestHK command success\n");
                }
                else
                {
                    OS_printf("CLIENT_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_CLIENT:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = CLIENT_RequestData(&ClientUart, &ClientData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("CLIENT_RequestData command success\n");
                }
                else
                {
                    OS_printf("CLIENT_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = CLIENT_CommandDevice(&ClientUart, CLIENT_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    ClientUart.deviceString = CLIENT_CFG_STRING;
    ClientUart.handle = CLIENT_CFG_HANDLE;
    ClientUart.isOpen = PORT_CLOSED;
    ClientUart.baud = CLIENT_CFG_BAUDRATE_HZ;
    status = uart_init_port(&ClientUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", ClientUart.deviceString, ClientUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", ClientUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&ClientUart);

    #ifdef _NOS_ENGINE_LINK_
        nos_destroy_link();
    #endif

    OS_printf("Cleanly exiting client application...\n\n"); 
    return 1;
}
